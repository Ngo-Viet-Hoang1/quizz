...
export function createAttemptHistoryColumns(onViewScorecard: (attemptId: string) => void) {
  return columnHelper.columns([
    columnHelper.accessor('_id', {
      header: ({ column }) => <DataTableColumnHeader column={column} title="ATTEMPT REF" />,
      cell: ({ row }) => (
      ...
      cell: ({ row }) => (
        <span className="text-xs text-muted-foreground">
          {row.original.startedAt
            ? format(new Date(row.original.startedAt), 'MMM dd, yyyy HH:mm')
            : 'N/A'}
        </span>
      ),
    }),
