...
interface QuestionCardProps {
  ...
  onTextAnswerBlur: (questionId: string, text?: string) => void;
  ...
  return (
    <Card
      ...
      className={`rounded-2xl border bg-card transition-all duration-200 scroll-mt-28 ${
        isFlagged
          ? 'ring-2 ring-amber-400/50 border-amber-400/60 shadow-xs'
          : isAnswered
            ? 'border-sky-200 dark:border-sky-900/60 shadow-xs'
      ...
      <CardContent className="p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between mb-4 pb-3 border-b border-border/60 gap-3">
          <div className="flex flex-wrap items-center gap-2.5">
            <Badge className="bg-sky-600 hover:bg-sky-600 text-white font-bold text-xs px-3 py-1 rounded-lg">
              Question {qIdx + 1}
            </Badge>
        ...
        {isTextType ? (
          <div className="space-y-2">
            <textarea
              className="w-full min-h-28 p-4 rounded-xl border border-border bg-background text-sm resize-y focus:ring-2 focus:ring-sky-600 focus:border-sky-600 outline-none transition-all placeholder:text-muted-foreground/60"
              placeholder="Type your answer here..."
              value={textAnswer}
              onChange={(e) => onTextAnswerChange(question._id, e.target.value)}
              onBlur={(e) => onTextAnswerBlur(question._id, e.target.value)}
            />
          </div>
        ) : (
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl border text-left transition-all duration-150 cursor-pointer ${
                    isSelected
                      ? 'border-sky-600 bg-sky-50/60 dark:bg-sky-950/30 ring-1 ring-sky-600/20 font-semibold text-foreground'
                      : 'border-border/80 bg-background hover:border-sky-400/50 hover:bg-muted/30 text-foreground/90'
                  }`}
                >
                  <div
                    className={`flex size-7 sm:size-8 items-center justify-center rounded-lg text-xs font-bold shrink-0 transition-colors ${
                      isSelected ? 'bg-sky-600 text-white' : 'bg-muted text-muted-foreground'
                    }`}
                  >
