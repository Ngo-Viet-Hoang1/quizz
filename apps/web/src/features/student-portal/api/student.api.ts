...
// ─── All Classes in Organization (Browse & Search) ───
export function useAllClasses(params?: { page?: number; limit?: number; search?: string }) {
  const api = useApiClient();
  return useQuery({
...
// ─── Organization Members (resolve names/avatars) ───
export function useOrgMembers() {
  const api = useApiClient();
  return useQuery({
    queryKey: ['organizations', 'me', 'members'] as const,
    queryFn: () => api.get<OrgMemberProfile[]>('/organizations/me/members'),
    staleTime: 60_000,
  });
}
...
export function useJoinClass() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['student', 'all-classes'] });
    },
  });
}
...
export function useSaveAnswer(attemptId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['student', 'attempt', attemptId] });
    },
  });
}
